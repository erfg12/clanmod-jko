...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while ( client->timeResidual >= 1000 ) 
	{
		...
		if ( cm_autoprotecttime.integer != 0 && client->ps.duelInProgress == qfalse ) {
			if ( client->pers.cmd.forwardmove || client->pers.cmd.rightmove || 
				client->pers.cmd.upmove || (client->pers.cmd.buttons & (BUTTON_ATTACK|BUTTON_ALT_ATTACK)) ) {
				client->cmIdleTimer = cm_autoprotecttime.integer;
			}
			else if ( !client->pers.cmd.forwardmove || !client->pers.cmd.rightmove || 
				!client->pers.cmd.upmove || !(client->pers.cmd.buttons & (BUTTON_ATTACK|BUTTON_ALT_ATTACK)) ) {
					if (client->cmIdleTimer == 0) {
						client->ps.eFlags |= EF_INVULNERABLE;
						client->invulnerableTimer = level.time + Q3_INFINITE;
					}
					if (client->cmIdleTimer != 0){
						client->cmIdleTimer--;
					}
...
void ClientThink_real( gentity_t *ent ) {
	...
	int			i;
	usercmd_t	*ucmd;
	unsigned static int TimeReswitch = 0;
	...
	//JediDog
	if (TimeReswitch < level.time)
	{ 
		ClientUserinfoChanged(ent->client->ps.clientNum);
	}
	TimeReswitch = level.time + 500;
...
//		G_Printf("serverTime >>>>>\n" );
	...
	if (ent->client->pers.ampunish == qtrue) {
		ent->client->ps.pm_type = PM_FLOAT;
		ent->client->ps.weaponTime = 100;
	}

	if (level.meetingMode == qtrue || level.matchMode == qtrue){
	...
	if (ent->client->ps.duelInProgress)
	{
		...
		if (ent->client->ps.duelTime < level.time)
		{
			...
			{
				if (cm_duelbeginsaberoff.integer == 0) {
		...
		else if (duelAgainst->health < 1 || duelAgainst->client->ps.stats[STAT_HEALTH] < 1)
		{
			...
			/*trap_SendServerCommand( ent-g_entities, va("cp \"^7You survived with\n^1%d^7/^2%d\n\"",
			...
			if (ent->health > 0 && ent->client->ps.stats[STAT_HEALTH] > 0)
			{
				...
				//cm NOTE: FULL heal
				ent->client->ps.stats[STAT_HEALTH] = ent->health = ent->client->ps.stats[STAT_MAX_HEALTH] + 25;
			...
			*/
			...
			if (ent->health > 0 && ent->client->ps.stats[STAT_HEALTH] > 0)
			{
				if (cm_multiduels.integer==0){
					...
					trap_SendServerCommand( ent-g_entities, va("cp \"%s %s %s!\n\"", ent->client->pers.netname, G_GetStripEdString("SVINGAME", "PLDUELWINNER"), duelAgainst->client->pers.netname) );
					trap_SendServerCommand( duelAgainst-g_entities, va("cp \"%s %s %s!\n\"", ent->client->pers.netname, G_GetStripEdString("SVINGAME", "PLDUELWINNER"), duelAgainst->client->pers.netname) );
				}
			...
			{ //it was a draw, because we both managed to die in the same frame
				if (cm_multiduels.integer==0){
					...
					trap_SendServerCommand( duelAgainst-g_entities, va("cp \"%s\n\"", G_GetStripEdString("SVINGAME", "PLDUELTIE")) );
				}
				trap_SendServerCommand( -1, va("cp \"%s\n\"", G_GetStripEdString("SVINGAME", "PLDUELTIE")) );
			}
		}
