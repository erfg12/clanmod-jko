...
int PassStandardEnemyChecks(bot_state_t *bs, gentity_t *en)
{
	...
	if (botstates[en->s.number] && cm_botsattackhumans.integer == 1)
	{
		return 0;
	}
...
void BotDamageNotification(gclient_t *bot, gentity_t *attacker)
{
	...
	if (bs_a)
	{
		...
		if (cm_botsattackhumans.integer == 1){
			return;
		}
...
//NOTE: This is base for cm_botsattackhumans
int PassLovedOneCheck(bot_state_t *bs, gentity_t *ent)
{
	int i;
	bot_state_t *loved;
...
int ScanForEnemies(bot_state_t *bs)
{
	...
	if (bs->currentEnemy && bs->currentEnemy->client &&
		!botstates[bs->currentEnemy->s.number] && cm_botsattackhumans.integer == 1)
	{ //dont attack other bots
		return -1;
	}
...
int BotAISetup( int restart ) {
	...
	//cm NOTE: I changed this for beta purposes
	//trap_Cvar_Register(&bot_honorableduelacceptance, "bot_honorableduelacceptance", "0", CVAR_CHEAT);
	trap_Cvar_Register(&bot_honorableduelacceptance, "bot_honorableduelacceptance", "0", 0);
#ifdef _DEBUG
