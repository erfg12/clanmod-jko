...
extern qboolean G_CheckMaxConnections( char *from );
...
void respawn( gentity_t *ent ) {
	...
	if (ent->client->pers.amempower == qtrue){
	...
	} else if (ent->client->pers.amterminator == qtrue) {
		for ( t = 0 ; t < MAX_WEAPONS ; t++ ) {
			ent->client->ps.ammo[t] = num;
		}
		ent->client->ps.weapon = WP_STUN_BATON;
		ent->client->ps.forceRestricted = qtrue;
		ent->client->ps.stats[STAT_WEAPONS] &= ~( 1 << WP_SABER );
		ent->client->ps.stats[STAT_WEAPONS] |= (1 << WP_STUN_BATON) | (1 << WP_BRYAR_PISTOL) | (1 << WP_BLASTER) | (1 << WP_DISRUPTOR)
			 | (1 << WP_BOWCASTER) | (1 << WP_REPEATER) | (1 << WP_DEMP2) | (1 << WP_FLECHETTE) | (1 << WP_ROCKET_LAUNCHER) | (1 << WP_THERMAL)
			 | (1 << WP_TRIP_MINE) | (1 << WP_DET_PACK);
		ent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= (1 << HI_BINOCULARS) | (1 << HI_SEEKER) | (1 << HI_MEDPAC) | (1 << HI_SHIELD) | (1 << HI_SENTRY_GUN);
	}
	if ( cm_autoprotecttime.integer != 0 ) {
		ent->client->cmIdleTimer = cm_autoprotecttime.integer;
	}
	...
	if (ent->client->pers.amsleep == qtrue) {
		...
		ent->client->ps.forceHandExtendTime = Q3_INFINITE;
...
team_t PickTeam( int ignoreClientNum ) {
	...
	if (( counts[TEAM_BLUE] > counts[TEAM_RED] ) && (!level.isLockedred) ){
		return TEAM_RED;
	}
	
	if (( counts[TEAM_RED] > counts[TEAM_BLUE] )&& (!level.isLockedblue) ) {
		return TEAM_BLUE;
	}
	
	// equal team count, so join the team with the lowest score
	if (( level.teamScores[TEAM_BLUE] > level.teamScores[TEAM_RED] ) && (!level.isLockedred) ){
		return TEAM_RED;
	}
	...
	if (!level.isLockedblue){
		...
		return TEAM_SPECTATOR; // We gotta go SOMEWHERE! mine as well be spec.
	}
}
...
static void ClientCleanName( const char *in, char *out, int outSize ) {
	...
	*p = 0;
	...
	while( 1 ) {
		...
		if( (colorlessLen == 0 || !*p) && ch == ' ' ) { //JAKES NOTE: Color + Space is not a character
			continue;
		}

		if (ch == 255 || ch == -96){ //JAKES NOTE: Character 255 is a blank character (ALT+255)
			continue;
		}
		...
		if( ch == Q_COLOR_ESCAPE ) {
			...
			//cm NOTE: I want to allow black names sometimes
			if (cm_blacknames.integer == 0) {
				if( ColorIndex(*in) == 0 ) {
					in++;
					continue;
				}
...
void ClientUserinfoChanged( int clientNum ) {
	...
	if (ent->client->TimeReswitch < level.time && ent->client->TimeReswitch > 0 && ent->client->TimeReswitch > level.time-500) {
		trap_SendServerCommand(clientNum, va("print\"^3Don't Flood. ^5Allow ^21 ^5second between changes.\n\""));
		return;
	}
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	char		*value;
	char		*value2;	//cm BanClients
//	char		*areabits;
	gclient_t	*client;
	char		userinfo[MAX_INFO_STRING];
	char tmpIP[22];			//cm BanClients
	gentity_t	*ent;
	gentity_t	*te;
	int			i;			//cm BanClients
	...
	if ( firstTime ) {
		if (strcmp(tmpIP, "") == 0 && !isBot) {
			return "You don't have an IP address!";
		}
	}
	...
	}//JediDog: End of fake players detection
...
*/
	...
	if (cm_savedualblade.integer == 1 && ent->client->pers.dualBlade == qtrue) {
...
void ClientDisconnect( int clientNum ) {
	...
	//cm NOTE: Don't forget to add persistant flags here
	ent->client->pers.amempower = qfalse;
	ent->client->pers.amterminator = qfalse;
