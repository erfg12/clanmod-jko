...
//Version modified to allow special settings like "g_maxforcerank 200".
//JediDog: Anti Force Crash
char  *GENERIC_FORCE    = "7-1-033330000000000333";
char  *FORCE_LOWER     = "-1-000000000000000000";
char  *FORCE_UPPER     = "-2-333333333333333333";
char* CheckForceStringTail(char* s, char* S) {
	char *p = s, *pu = FORCE_UPPER, *pl = FORCE_LOWER;
	if (!s || strlen(s) != 21) return GENERIC_FORCE;
	while(*p) {if (*p > *pu++ || *p++ < *pl++) {return GENERIC_FORCE;}}
	return S;
}
char* CheckForceString(char* S) {
	int i, l; char *p = S;
	if (!S || (l = strlen(S)) > 24 || l < 22  || *S < '0' || *S > '9') return GENERIC_FORCE;
	for(i = 1 ; i <= 3 ; i++) {
		if (S[i] == '-') return CheckForceStringTail(S+i, S);
		if (S[i] < '0' || S[i] > '9') break;
	} return GENERIC_FORCE;
}//end of anti force crash
...
void WP_InitForcePowers( gentity_t *ent )
{
    ...
    char* temp; //JediDog: Anti Force Crash
...
void ForceThrow( gentity_t *self, qboolean pull )
{
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) 
	{
		...
		{
			if ( mod_pushall.integer || ent->s.eType != ET_ITEM )
			{
	...
	if ( ent_count )
	{
		...
		for ( x = 0; x < ent_count; x++ )
		{
			...
			else if ( mod_pushall.integer && push_list[x]->itemtype != IT_WEAPON && push_list[x]->s.eType == ET_ITEM )//rolling and stationary thermal detonators are dealt with below
			{
...
void WP_ForcePowersUpdate( gentity_t *self, usercmd_t *ucmd )
{
	...
	if ((self->client->pers.ampunish == qtrue) || (self->client->ps.fd.forceGripBeingGripped > level.time))
	{
