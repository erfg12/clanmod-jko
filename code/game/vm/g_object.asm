...
;270:		{
line 271
;271:			DoImpact( ent, traceEnt, qtrue );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 1080
...
line 275
;274:
;275:	if ( !ent || (ent->takedamage&&ent->health <= 0) )
ADDRLP4 1132
ADDRFP4 0
INDIRP4
ASGNP4
...
LABELV $182
line 276
;276:	{//been destroyed by impact
line 279
;277:		//chunks?
;278://		G_Sound( ent, G_SoundIndex( "sound/movers/objects/objectBreak.wav" ) );
;279:		return;
ADDRGP4 $133
JUMPV
LABELV $180
...
;280:	}
;281:
;282:	//do impact physics
;283:	if ( ent->s.pos.trType == TR_GRAVITY )//tr.fraction < 1.0 && 
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
NEI4 $183
line 284
;284:	{//FIXME: only do this if no trDelta
line 285
;285:		if ( g_gravity.value <= 0 || tr.plane.normal[2] < 0.7 )
ADDRGP4 g_gravity+8
INDIRF4
CNSTF4 0
LEF4 $190
...
;286:		{
line 287
;287:			if ( ent->s.eFlags&(EF_BOUNCE|EF_BOUNCE_HALF) )
ADDRFP4 0
INDIRP4
CNSTI4 8
ADDP4
...
;290:				{
line 291
;291:					VectorCopy( tr.endpos, ent->r.currentOrigin );
ADDRFP4 0
INDIRP4
CNSTI4 368
ADDP4
...
ASGNB 12
line 292
;292:					VectorCopy( tr.endpos, ent->s.pos.trBase );
ADDRFP4 0
INDIRP4
CNSTI4 24
ADDP4
...
ASGNB 12
line 293
;293:					VectorClear( ent->s.pos.trDelta );
ADDRLP4 1140
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNF4
line 294
;294:					ent->s.pos.trTime = level.time;
ADDRFP4 0
INDIRP4
CNSTI4 16
ADDP4
...
;307:		{
line 308
;308:			ent->s.apos.trType = TR_STATIONARY;
ADDRFP4 0
INDIRP4
CNSTI4 48
ADDP4
...
ASGNI4
line 309
;309:			pitch_roll_for_slope( ent, tr.plane.normal );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 0+24
...
pop
line 312
;310:			//ent->r.currentAngles[0] = 0;//FIXME: match to slope
;311:			//ent->r.currentAngles[2] = 0;//FIXME: match to slope
;312:			VectorCopy( ent->r.currentAngles, ent->s.apos.trBase );
ADDRLP4 1140
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNB 12
line 316
;313:			//okay, we hit the floor, might as well stop or prediction will
;314:			//make us go through the floor!
;315:			//FIXME: this means we can't fall if something is pulled out from under us...
;316:			G_StopObjectMoving( ent );
ADDRFP4 0
INDIRP4
ARGP4
...
LABELV $183
line 319
;319:	else if (ent->s.weapon != WP_SABER)
ADDRFP4 0
INDIRP4
CNSTI4 276
ADDP4
...
;320:	{
line 321
;321:		ent->s.apos.trType = TR_STATIONARY;
ADDRFP4 0
INDIRP4
CNSTI4 48
ADDP4
...
ASGNI4
line 322
;322:		pitch_roll_for_slope( ent, tr.plane.normal );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 0+24
...
pop
line 325
;323:		//ent->r.currentAngles[0] = 0;//FIXME: match to slope
;324:		//ent->r.currentAngles[2] = 0;//FIXME: match to slope
;325:		VectorCopy( ent->r.currentAngles, ent->s.apos.trBase );
ADDRLP4 1140
ADDRFP4 0
INDIRP4
ASGNP4
...
;327:
;328:	//call touch func
;329:	ent->touch( ent, &g_entities[tr.entityNum], &tr );
ADDRLP4 1140
ADDRFP4 0
INDIRP4
ASGNP4
...
;331:
;332:
;333:void G_StopObjectMoving( gentity_t *object )
;334:{
line 335
;335:	object->s.pos.trType = TR_STATIONARY;
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
ASGNI4
line 336
;336:	VectorCopy( object->r.currentOrigin, object->s.origin );
ADDRLP4 0
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNB 12
line 337
;337:	VectorCopy( object->r.currentOrigin, object->s.pos.trBase );
ADDRLP4 4
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNB 12
line 338
;338:	VectorClear( object->s.pos.trDelta );
ADDRLP4 8
ADDRFP4 0
INDIRP4
ASGNP4
...
;340:	/*
;341:	//Stop spinning
;342:	VectorClear( self->s.apos.trDelta );
;343:	vectoangles(trace->plane.normal, self->s.angles);
;344:	VectorCopy(self->s.angles, self->r.currentAngles );
;345:	VectorCopy(self->s.angles, self->s.apos.trBase);
;346:	*/
;347:}
LABELV $204
...
line 350
;348:
;349:void G_StartObjectMoving( gentity_t *object, vec3_t dir, float speed, trType_t trType )
;350:{
line 351
...
line 354
;352:
;353:	//object->s.eType = ET_GENERAL;
;354:	object->s.pos.trType = trType;
ADDRFP4 0
INDIRP4
CNSTI4 12
ADDP4
...
ASGNI4
line 355
;355:	VectorCopy( object->r.currentOrigin, object->s.pos.trBase );
ADDRLP4 0
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNB 12
line 356
;356:	VectorScale(dir, speed, object->s.pos.trDelta );
ADDRFP4 0
INDIRP4
CNSTI4 36
ADDP4
...
ASGNF4
line 357
;357:	object->s.pos.trTime = level.time;
ADDRFP4 0
INDIRP4
CNSTI4 16
ADDP4
...
;358:
;359:	/*
;360:	//FIXME: incorporate spin?
;361:	vectoangles(dir, object->s.angles);
;362:	VectorCopy(object->s.angles, object->s.apos.trBase);
;363:	VectorSet(object->s.apos.trDelta, 300, 0, 0 );
;364:	object->s.apos.trTime = level.time;
;365:	*/
;366:
;367:	//FIXME: make these objects go through G_RunObject automatically, like missiles do
;368:	if ( object->think == NULL )
ADDRFP4 0
INDIRP4
CNSTI4 636
ADDP4
...
;369:	{
line 370
;370:		object->nextthink = level.time + FRAMETIME;
ADDRFP4 0
INDIRP4
CNSTI4 632
ADDP4
...
line 374
;373:	else
;374:	{//You're responsible for calling RunObject
line 375
;375:	}
LABELV $208
line 376
