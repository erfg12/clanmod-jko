...
;1389:		{
line 1390
;1390:			trap_SendConsoleCommand("sv_saberswitch");
ADDRGP4 $837
ARGP4
...
;1393:	}
;1394:
;1395:	//rww - hack to make weapon numbers same as single player
;1396:	if (num > WP_STUN_BATON)
ADDRLP4 0
INDIRI4
CNSTI4 1
LEI4 $838
...
;1401:	{
line 1402
;1402:		if (cg.snap->ps.stats[STAT_WEAPONS] & (1 << WP_SABER))
ADDRGP4 cg+36
INDIRP4
CNSTI4 276
ADDP4
...
line 1412
;1411:
;1412:	if (num > WP_DET_PACK+1)
ADDRLP4 0
INDIRI4
CNSTI4 14
LEI4 $843
line 1413
;1413:	{ //other weapons are off limits due to not actually being weapon weapons
line 1414
;1414:		return;
ADDRGP4 $818
JUMPV
...
line 1421
;1420:
;1421:		if (cg.snap->ps.weapon >= WP_THERMAL &&
ADDRLP4 24
CNSTI4 192
ASGNI4
ADDRGP4 cg+36
...
GTI4 $847
line 1423
;1422:			cg.snap->ps.weapon <= WP_DET_PACK)
;1423:		{
line 1425
;1424:			// already in cycle range so start with next cycle item
;1425:			weap = cg.snap->ps.weapon + 1;
ADDRLP4 16
ADDRGP4 cg+36
INDIRP4
CNSTI4 192
...
;1428:		{
line 1430
;1429:			// not in cycle range, so start with thermal detonator
;1430:			weap = WP_THERMAL;
ADDRLP4 16
CNSTI4 11
ASGNI4
line 1431
...
line 1435
;1432:
;1433:		// prevent an endless loop
;1434:		while ( i <= 4 )
;1435:		{
line 1436
;1436:			if (weap > WP_DET_PACK)
ADDRLP4 16
INDIRI4
CNSTI4 13
LEI4 $855
...
line 1441
;1440:
;1441:			if (CG_WeaponSelectable(weap))
ADDRLP4 16
INDIRI4
ARGI4
ADDRLP4 28
...
line 1452
;1451:
;1452:	if (!CG_WeaponSelectable(num))
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 16
...
;1455:	}
;1456:
;1457:	cg.weaponSelectTime = cg.time;
ADDRGP4 cg+13372
ADDRGP4 cg+64
INDIRI4
ASGNI4
line 1459
;1458:
;1459:	if ( ! ( cg.snap->ps.stats[STAT_WEAPONS] & ( 1 << num ) ) )
ADDRGP4 cg+36
INDIRP4
CNSTI4 276
ADDP4
...
;1460:	{
line 1461
;1461:		if (num == WP_SABER)
ADDRLP4 0
INDIRI4
CNSTI4 2
NEI4 $818
line 1462
;1462:		{ //don't have saber, try the stun baton on the same slot
line 1463
;1463:			num = WP_STUN_BATON;
ADDRLP4 0
CNSTI4 1
ASGNI4
line 1465
;1464:
;1465:			if ( ! ( cg.snap->ps.stats[STAT_WEAPONS] & ( 1 << num ) ) )
ADDRGP4 cg+36
INDIRP4
CNSTI4 276
ADDP4
...
;1471:		{
line 1472
;1472:			return;		// don't have the weapon
LABELV $867
line 1474
;1473:		}
;1474:	}
...
;1477:	{
line 1478
;1478:		trap_S_MuteSound(cg.snap->ps.clientNum, CHAN_WEAPON);
ADDRGP4 cg+36
INDIRP4
CNSTI4 188
ADDP4
...
;1484:/*
;1485:===================
;1486:CG_OutOfAmmoChange
;1487:
;1488:The current weapon has just run out of ammo
;1489:===================
;1490:*/
;1491:void CG_OutOfAmmoChange( int oldWeapon )
;1492:{
line 1495
;1493:	int		i;
;1494:
;1495:	cg.weaponSelectTime = cg.time;
ADDRGP4 cg+13372
ADDRGP4 cg+64
INDIRI4
ASGNI4
line 1497
;1496:
;1497:	for ( i = WP_DET_PACK ; i > 0 ; i-- )	//We don't want the emplaced or turret
ADDRLP4 0
CNSTI4 13
ASGNI4
LABELV $879
...
line 1506
;1501:			/*
;1502:			if ( 1 == cg_autoswitch.integer && 
;1503:				( i == WP_TRIP_MINE || i == WP_DET_PACK || i == WP_THERMAL || i == WP_ROCKET_LAUNCHER) ) // safe weapon switch
;1504:			*/
;1505:			//rww - Don't we want to make sure i != one of these if autoswitch is 1 (safe)?
;1506:			if (cg_autoswitch.integer != 1 || (i != WP_TRIP_MINE && i != WP_DET_PACK && i != WP_THERMAL && i != WP_ROCKET_LAUNCHER))
...
EQI4 $889
line 1509
;1509:				{ //don't even do anything if we're just selecting the weapon we already have/had
line 1510
;1510:					cg.weaponSelect = i;
ADDRGP4 cg+3508
ADDRLP4 0
INDIRI4
ASGNI4
...
line 1517
;1516:
;1517:	trap_S_MuteSound(cg.snap->ps.clientNum, CHAN_WEAPON);
ADDRGP4 cg+36
INDIRP4
CNSTI4 188
ADDP4
...
;1528:*/
;1529:
;1530:void CG_FireATST(centity_t *cent, qboolean altFire)
;1531:{
line 1563
;1532://No muzzle flash for now?
;1533:#if 0
;1534:	int getMeBolt = 0;
;1535:	mdxaBone_t boltMatrix;
;1536:	vec3_t flashorigin, flashdir;
;1537:
;1538:	if (!cgs.clientinfo[cent->currentState.number].bolt_lhand)
;1539:	{
;1540:		return;
;1541:	}
;1542:
;1543:	if (altFire)
;1544:	{
;1545:		getMeBolt = cgs.clientinfo[cent->currentState.number].bolt_lhand;
;1546:	}
;1547:	else
;1548:	{
;1549:		getMeBolt = cgs.clientinfo[cent->currentState.number].bolt_rhand;
;1550:	}
;1551:
;1552: 	if (!(trap_G2API_GetBoltMatrix(cent->ghoul2, 0, getMeBolt, &boltMatrix, cent->lerpAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale)))
;1553:	{
;1554:		return;
;1555:	}
;1556:			
;1557:	trap_G2API_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, flashorigin);
;1558:	trap_G2API_GiveMeVectorFromMatrix(&boltMatrix, POSITIVE_X, flashdir);
;1559:
;1560:	trap_FX_PlayEffectID(trap_FX_RegisterEffect("turret/muzzle_flash.efx"), flashorigin, flashdir);
;1561:#endif
;1562:
;1563:	trap_S_StartSound(NULL, cent->currentState.number, CHAN_WEAPON, trap_S_RegisterSound(va("sound/weapons/atst/ATSTfire1.wav"/*, Q_irand(1,4)*/)));
ADDRGP4 $894
ARGP4
ADDRLP4 0
ADDRGP4 va
...
line 1567
;1565:
;1566:void CG_GetClientWeaponMuzzleBoltPoint(int clIndex, vec3_t to)
;1567:{
line 1571
;1568:	centity_t *cent;
;1569:	mdxaBone_t	boltMatrix;
;1570:
;1571:	if (clIndex < 0 || clIndex >= MAX_CLIENTS)
ADDRLP4 52
ADDRFP4 0
INDIRI4
ASGNI4
...
line 1578
;1577:
;1578:	if (!cent || !cent->ghoul2 || !trap_G2_HaveWeGhoul2Models(cent->ghoul2) ||
ADDRLP4 60
CNSTU4 0
ASGNU4
ADDRLP4 0
...
LABELV $903
line 1580
;1579:		!trap_G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1))
;1580:	{
line 1581
;1581:		return;
ADDRGP4 $895
...
;1582:	}
;1583:
;1584:	trap_G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &boltMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
ADDRLP4 0
INDIRP4
CNSTI4 952
ADDP4
...
pop
line 1585
;1585:	trap_G2API_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, to);
ADDRLP4 4
ARGP4
CNSTI4 0
ARGI4
...
;1590:CG_FireWeapon
;1591:
;1592:Caused by an EV_FIRE_WEAPON event
;1593:================
;1594:*/
;1595:void CG_FireWeapon( centity_t *cent, qboolean altFire ) {
line 1600
;1596:	entityState_t *ent;
;1597:	int				c;
;1598:	weaponInfo_t	*weap;
;1599:
;1600:	if (cent->isATST)
ADDRFP4 0
INDIRP4
...
;1604:	}
;1605:
;1606:	ent = &cent->currentState;
ADDRLP4 8
ADDRFP4 0
INDIRP4
ASGNP4
line 1607
;1607:	if ( ent->weapon == WP_NONE ) {
ADDRLP4 8
INDIRP4
CNSTI4 276
ADDP4
...
line 1610
;1609:	}
;1610:	if ( ent->weapon >= WP_NUM_WEAPONS ) {
ADDRLP4 8
INDIRP4
CNSTI4 276
ADDP4
...
LTI4 $911
line 1611
;1611:		CG_Error( "CG_FireWeapon: ent->weapon >= WP_NUM_WEAPONS" );
ADDRGP4 $913
ARGP4
ADDRGP4 CG_Error
CALLV
...
line 1614
;1613:	}
;1614:	weap = &cg_weapons[ ent->weapon ];
ADDRLP4 4
CNSTI4 208
ADDRLP4 8
INDIRP4
...
line 1618
;1615:
;1616:	// mark the entity as muzzle flashing, so when it is added it will
;1617:	// append the flash to the weapon model
;1618:	cent->muzzleFlashTime = cg.time;
ADDRFP4 0
INDIRP4
CNSTI4 600
ADDP4
...
line 1620
;1619:
;1620:	if (cg.predictedPlayerState.clientNum == cent->currentState.number)
ADDRGP4 cg+96+144
INDIRI4
ADDRFP4 0
INDIRP4
...
;1621:	{
line 1622
;1622:		if ((ent->weapon == WP_BRYAR_PISTOL && altFire) ||
ADDRLP4 8
INDIRP4
CNSTI4 276
ADDP4
...
LABELV $923
line 1625
;1623:			(ent->weapon == WP_BOWCASTER && !altFire) ||
;1624:			(ent->weapon == WP_DEMP2 && altFire))
;1625:		{
line 1626
;1626:			float val = ( cg.time - cent->currentState.constantLight ) * 0.001f;
ADDRLP4 12
CNSTF4 981668463
ADDRGP4 cg+64
INDIRI4
...
LABELV $919
line 1641
;1641:		else if (ent->weapon == WP_ROCKET_LAUNCHER ||
ADDRLP4 12
ADDRLP4 8
INDIRP4
CNSTI4 276
...
LABELV $933
line 1644
;1642:			(ent->weapon == WP_REPEATER && altFire) ||
;1643:			ent->weapon == WP_FLECHETTE)
;1644:		{
line 1645
;1645:			if (ent->weapon == WP_ROCKET_LAUNCHER)
ADDRLP4 8
INDIRP4
CNSTI4 276
ADDP4
...
;1646:			{
line 1647
;1647:				CGCam_Shake(Q_irand(2, 3), 350);
CNSTI4 2
ARGI4
CNSTI4 3
ARGI4
...
LABELV $935
line 1649
;1649:			else if (ent->weapon == WP_REPEATER)
ADDRLP4 8
INDIRP4
CNSTI4 276
ADDP4
...
;1650:			{
line 1651
;1651:				CGCam_Shake(Q_irand(2, 3), 350);
CNSTI4 2
ARGI4
CNSTI4 3
ARGI4
...
LABELV $937
line 1653
;1653:			else if (ent->weapon == WP_FLECHETTE)
ADDRLP4 8
INDIRP4
CNSTI4 276
ADDP4
...
;1656:				{
line 1657
;1657:					CGCam_Shake(Q_irand(2, 3), 350);
CNSTI4 2
ARGI4
CNSTI4 3
ARGI4
...
LABELV $915
line 1667
;1666:	// lightning gun only does this this on initial press
;1667:	if ( ent->weapon == WP_DEMP2 ) {
ADDRLP4 8
INDIRP4
CNSTI4 276
ADDP4
...
NEI4 $943
line 1668
;1668:		if ( cent->pe.lightningFiring ) {
ADDRFP4 0
INDIRP4
CNSTI4 844
ADDP4
...
line 1674
;1672:
;1673:	// play quad sound if needed
;1674:	if ( cent->currentState.powerups & ( 1 << PW_QUAD ) ) {
ADDRFP4 0
INDIRP4
CNSTI4 272
ADDP4
...
EQI4 $947
line 1676
;1675:		//trap_S_StartSound (NULL, cent->currentState.number, CHAN_ITEM, cgs.media.quadSound );
;1676:	}
LABELV $947
line 1680
;1677:
...
;1691:			{
line 1692
;1692:				trap_S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->altFlashSound[c] );
CNSTP4 0
ARGP4
ADDRLP4 8
INDIRP4
...
;1694:		}
line 1699
;1695://		if ( weap->altFlashSnd )
;1696://		{
;1697://			trap_S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->altFlashSnd );
;1698://		}
;1699:	}
ADDRGP4 $950
JUMPV
...
;1711:			{
line 1712
;1712:				trap_S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->flashSound[c] );
CNSTP4 0
ARGP4
ADDRLP4 8
INDIRP4
...
;1719:/*
;1720:=================
;1721:CG_MissileHitWall
;1722:
;1723:Caused by an EV_MISSILE_MISS event, or directly by local bullet tracing
;1724:=================
;1725:*/
;1726:void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType, qboolean altFire, int charge) 
;1727:{
line 1729
;1728:	int parm;
;1729:	vec3_t up={0,0,1};
ADDRLP4 0
ADDRGP4 $972
INDIRB
ASGNB 12
...
ASGNI4
line 1737
;1737:			FX_BryarAltHitWall( origin, dir, parm );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
;1740:		{
line 1741
;1741:			FX_BryarHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
;1744:
;1745:	case WP_TURRET:
;1746:		FX_TurretHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
line 1750
;1748:
;1749:	case WP_BLASTER:
;1750:		FX_BlasterWeaponHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
line 1754
;1752:
;1753:	case WP_DISRUPTOR:
;1754:		FX_DisruptorAltMiss( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
line 1758
;1756:
;1757:	case WP_BOWCASTER:
;1758:		FX_BowcasterHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
;1763:		{
line 1764
;1764:			FX_RepeaterAltHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
;1767:		{
line 1768
;1768:			FX_RepeaterHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
;1774:		{
line 1775
;1775:			trap_FX_PlayEffectID(trap_FX_RegisterEffect("demp2/altDetonate.efx"), origin, dir);
ADDRGP4 $988
ARGP4
ADDRLP4 20
...
;1778:		{
line 1779
;1779:			FX_DEMP2_HitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
line 1790
;1782:
;1783:	case WP_FLECHETTE:
;1784:		/*if (altFire)
;1785:		{
;1786:			CG_SurfaceExplosion(origin, dir, 20.0f, 12.0f, qtrue);
;1787:		}
;1788:		else
;1789:		*/
;1790:		if (!altFire)
...
;1791:		{
line 1792
;1792:			FX_FlechetteWeaponHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
line 1797
;1795:
;1796:	case WP_ROCKET_LAUNCHER:
;1797:		FX_RocketHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
line 1801
;1799:
;1800:	case WP_THERMAL:
;1801:		trap_FX_PlayEffectID( cgs.effects.thermalExplosionEffect, origin, dir );
...
pop
line 1802
;1802:		trap_FX_PlayEffectID( cgs.effects.thermalShockwaveEffect, origin, up );
...
line 1806
;1804:
;1805:	case WP_EMPLACED_GUN:
;1806:		FX_BlasterWeaponHitWall( origin, dir );
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
...
pop
line 1808
;1807:		//FIXME: Give it its own hit wall effect
;1808:		break;
LABELV $973
LABELV $974
line 1810
...
;1813:/*
;1814:=================
;1815:CG_MissileHitPlayer
;1816:=================
;1817:*/
;1818:void CG_MissileHitPlayer(int weapon, vec3_t origin, vec3_t dir, int entityNum, qboolean altFire) 
;1819:{
line 1820
;1820:	qboolean	humanoid = qtrue;
ADDRLP4 0
CNSTI4 1
ASGNI4
line 1821
;1821:	vec3_t up={0,0,1};
ADDRLP4 4
ADDRGP4 $1002
INDIRB
ASGNB 12
...
;1822:
;1823:	/*
;1824:	// NOTENOTE Non-portable code from single player
;1825:	if ( cent->gent )
;1826:	{
;1827:		other = &g_entities[cent->gent->s.otherEntityNum];
;1828:
;1829:		if ( other->client && other->client->playerTeam == TEAM_BOTS )
;1830:		{
;1831:			humanoid = qfalse;
;1832:		}
;1833:	}
;1834:	*/	
;1835:
;1836:	// NOTENOTE No bleeding in this game
;1837://	CG_Bleed( origin, entityNum );
;1838:
;1839:	// some weapons will make an explosion with the blood, while
;1840:	// others will just make the blood
;1841:	switch ( weapon ) {
ADDRLP4 16
ADDRFP4 0
INDIRI4
...
;1844:		{
line 1845
;1845:			FX_BryarAltHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
;1848:		{
line 1849
;1849:			FX_BryarHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
;1852:
;1853:	case WP_TURRET:
;1854:		FX_TurretHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
line 1858
;1856:
;1857:	case WP_BLASTER:
;1858:		FX_BlasterWeaponHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
line 1862
;1860:
;1861:	case WP_DISRUPTOR:
;1862:		FX_DisruptorAltHit( origin, dir);
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
line 1866
;1864:
;1865:	case WP_BOWCASTER:
;1866:		FX_BowcasterHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
;1871:		{
line 1872
;1872:			FX_RepeaterAltHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
;1875:		{
line 1876
;1876:			FX_RepeaterHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
;1879:
;1880:	case WP_DEMP2:
;1881:		// Do a full body effect here for some more feedback
;1882:		// NOTENOTE The chaining of the demp2 is not yet implemented.
;1883:		/*
;1884:		if ( other )
;1885:		{
;1886:			other->s.powerups |= ( 1 << PW_DISINT_1 );
;1887:			other->client->ps.powerups[PW_DISINT_1] = cg.time + 650;
;1888:		}
;1889:		*/
;1890:		if (altFire)
ADDRFP4 16
...
;1891:		{
line 1892
;1892:			trap_FX_PlayEffectID(trap_FX_RegisterEffect("demp2/altDetonate.efx"), origin, dir);
ADDRGP4 $988
ARGP4
ADDRLP4 20
ADDRGP4 trap_FX_RegisterEffect
...
;1895:		{
line 1896
;1896:			FX_DEMP2_HitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
line 1901
;1899:
;1900:	case WP_FLECHETTE:
;1901:		FX_FlechetteWeaponHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
line 1905
;1903:
;1904:	case WP_ROCKET_LAUNCHER:
;1905:		FX_RocketHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
line 1909
;1907:
;1908:	case WP_THERMAL:
;1909:		trap_FX_PlayEffectID( cgs.effects.thermalExplosionEffect, origin, dir );
ADDRGP4 cgs+71564+132
INDIRI4
ARGI4
ADDRFP4 4
...
pop
line 1910
;1910:		trap_FX_PlayEffectID( cgs.effects.thermalShockwaveEffect, origin, up );
ADDRGP4 cgs+71564+136
INDIRI4
ARGI4
ADDRFP4 4
...
LABELV $1025
line 1914
;1912:	case WP_EMPLACED_GUN:
;1913:		//FIXME: Its own effect?
;1914:		FX_BlasterWeaponHitPlayer( origin, dir, humanoid );
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
...
;1935:===============
;1936:*/
;1937:void CG_Tracer( vec3_t source, vec3_t dest ) {
line 1946
;1938:	vec3_t		forward, right;
;1939:	polyVert_t	verts[4];
;1940:	vec3_t		line;
;1941:	float		len, begin, end;
;1942:	vec3_t		start, finish;
;1943:	vec3_t		midpoint;
;1944:
;1945:	// tracer
;1946:	VectorSubtract( dest, source, forward );
ADDRLP4 180
ADDRFP4 4
INDIRP4
ASGNP4
...
ASGNF4
line 1947
;1947:	len = VectorNormalize( forward );
ADDRLP4 108
ARGP4
ADDRLP4 196
...
line 1950
;1948:
;1949:	// start at least a little ways from the muzzle
;1950:	if ( len < 100 ) {
ADDRLP4 164
INDIRF4
CNSTF4 1120403456
...
ASGNF4
line 1954
;1954:	end = begin + cg_tracerLength.value;
ADDRLP4 156
ADDRLP4 160
INDIRF4
...
LABELV $1034
line 1958
;1958:	VectorMA( source, begin, forward, start );
ADDRLP4 204
ADDRFP4 0
INDIRP4
ASGNP4
...
ASGNF4
line 1959
;1959:	VectorMA( source, end, forward, finish );
ADDRLP4 212
ADDRFP4 0
INDIRP4
ASGNP4
...
line 1961
;1960:
;1961:	line[0] = DotProduct( forward, cg.refdef.viewaxis[1] );
ADDRLP4 144
ADDRLP4 108
INDIRF4
ADDRGP4 cg+3616+36+12
...
ASGNF4
line 1962
;1962:	line[1] = DotProduct( forward, cg.refdef.viewaxis[2] );
ADDRLP4 144+4
ADDRLP4 108
INDIRF4
ADDRGP4 cg+3616+36+24
...
line 1964
;1963:
;1964:	VectorScale( cg.refdef.viewaxis[1], line[1], right );
ADDRLP4 96
ADDRGP4 cg+3616+36+12
INDIRF4
ADDRLP4 144+4
...
ASGNF4
line 1965
;1965:	VectorMA( right, -line[0], cg.refdef.viewaxis[2], right );
ADDRLP4 220
ADDRLP4 144
INDIRF4
NEGF4
...
line 1968
;1967:
;1968:	VectorMA( finish, cg_tracerWidth.value, right, verts[0].xyz );
ADDRLP4 0
ADDRLP4 132
INDIRF4
ADDRLP4 96
...
ASGNF4
line 1971
;1971:	verts[0].modulate[0] = 255;
ADDRLP4 0+20
CNSTU1 255
ASGNU1
line 1972
;1972:	verts[0].modulate[1] = 255;
ADDRLP4 0+20+1
CNSTU1 255
ASGNU1
line 1973
;1973:	verts[0].modulate[2] = 255;
ADDRLP4 0+20+2
CNSTU1 255
ASGNU1
line 1974
;1974:	verts[0].modulate[3] = 255;
ADDRLP4 0+20+3
CNSTU1 255
ASGNU1
line 1976
;1975:
;1976:	VectorMA( finish, -cg_tracerWidth.value, right, verts[1].xyz );
ADDRLP4 0+24
ADDRLP4 132
INDIRF4
ADDRLP4 96
...
ASGNF4
line 1979
;1979:	verts[1].modulate[0] = 255;
ADDRLP4 0+24+20
CNSTU1 255
ASGNU1
line 1980
;1980:	verts[1].modulate[1] = 255;
ADDRLP4 0+24+20+1
CNSTU1 255
ASGNU1
line 1981
;1981:	verts[1].modulate[2] = 255;
ADDRLP4 0+24+20+2
CNSTU1 255
ASGNU1
line 1982
;1982:	verts[1].modulate[3] = 255;
ADDRLP4 0+24+20+3
CNSTU1 255
ASGNU1
line 1984
;1983:
;1984:	VectorMA( start, -cg_tracerWidth.value, right, verts[2].xyz );
ADDRLP4 0+48
ADDRLP4 120
INDIRF4
ADDRLP4 96
...
ASGNF4
line 1987
;1987:	verts[2].modulate[0] = 255;
ADDRLP4 0+48+20
CNSTU1 255
ASGNU1
line 1988
;1988:	verts[2].modulate[1] = 255;
ADDRLP4 0+48+20+1
CNSTU1 255
ASGNU1
line 1989
;1989:	verts[2].modulate[2] = 255;
ADDRLP4 0+48+20+2
CNSTU1 255
ASGNU1
line 1990
;1990:	verts[2].modulate[3] = 255;
ADDRLP4 0+48+20+3
CNSTU1 255
ASGNU1
line 1992
;1991:
;1992:	VectorMA( start, cg_tracerWidth.value, right, verts[3].xyz );
ADDRLP4 0+72
ADDRLP4 120
INDIRF4
ADDRLP4 96
...
ASGNF4
line 1995
;1995:	verts[3].modulate[0] = 255;
ADDRLP4 0+72+20
CNSTU1 255
ASGNU1
line 1996
;1996:	verts[3].modulate[1] = 255;
ADDRLP4 0+72+20+1
CNSTU1 255
ASGNU1
line 1997
;1997:	verts[3].modulate[2] = 255;
ADDRLP4 0+72+20+2
CNSTU1 255
ASGNU1
line 1998
;1998:	verts[3].modulate[3] = 255;
ADDRLP4 0+72+20+3
CNSTU1 255
ASGNU1
line 2000
;1999:
;2000:	trap_R_AddPolyToScene( cgs.media.tracerShader, 4, verts );
...
line 2002
;2001:
;2002:	midpoint[0] = ( start[0] + finish[0] ) * 0.5;
ADDRLP4 168
CNSTF4 1056964608
ADDRLP4 120
INDIRF4
...
ASGNF4
line 2003
;2003:	midpoint[1] = ( start[1] + finish[1] ) * 0.5;
ADDRLP4 168+4
CNSTF4 1056964608
ADDRLP4 120+4
INDIRF4
...
ASGNF4
line 2004
;2004:	midpoint[2] = ( start[2] + finish[2] ) * 0.5;
ADDRLP4 168+8
CNSTF4 1056964608
ADDRLP4 120+8
INDIRF4
...
line 2009
;2005:
;2006:	// add the tracer sound
;2007:	//trap_S_StartSound( midpoint, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.tracerSound );
;2008:
;2009:}
LABELV $1028
...
;2011:/*
;2012:======================
;2013:CG_CalcMuzzlePoint
;2014:======================
;2015:*/
;2016:qboolean CG_CalcMuzzlePoint( int entityNum, vec3_t muzzle ) {
line 2022
;2017:	vec3_t		forward, right;
;2018:	vec3_t		gunpoint;
;2019:	centity_t	*cent;
;2020:	int			anim;
;2021:
;2022:	if ( entityNum == cg.snap->ps.clientNum )
ADDRFP4 0
INDIRI4
ADDRGP4 cg+36
INDIRP4
...
NEI4 $1214
line 2023
;2023:	{ //I'm not exactly sure why we'd be rendering someone else's crosshair, but hey.
line 2024
;2024:		int weapontype = cg.snap->ps.weapon;
ADDRLP4 56
ADDRGP4 cg+36
INDIRP4
CNSTI4 192
...
ASGNI4
line 2027
;2025:		vec3_t weaponMuzzle;
;2026:
;2027:		VectorCopy(WP_MuzzlePoint[weapontype], weaponMuzzle);
ADDRLP4 44
CNSTI4 12
ADDRLP4 56
INDIRI4
...
line 2029
;2028:
;2029:		if (weapontype == WP_DISRUPTOR || weapontype == WP_STUN_BATON || weapontype == WP_SABER)
ADDRLP4 56
INDIRI4
CNSTI4 5
EQI4 $1221
...
ASGNF4
line 2037
;2037:			weaponMuzzle[0] = 16;
ADDRLP4 44
CNSTF4 1098907648
ASGNF4
line 2038
;2038:			weaponMuzzle[2] = 128;
ADDRLP4 44+8
CNSTF4 1124073472
ASGNF4
line 2039
...
line 2041
;2040:
;2041:		if (cg.renderingThirdPerson)
ADDRGP4 cg+88
INDIRI4
CNSTI4 0
EQI4 $1230
...
;2042:		{
line 2043
;2043:			VectorCopy( cg.predictedPlayerEntity.lerpOrigin, gunpoint );
ADDRLP4 32
ADDRGP4 cg+1476+928
INDIRB
ASGNB 12
line 2044
;2044:			AngleVectors( cg.predictedPlayerEntity.lerpAngles, forward, right, NULL );
ADDRGP4 cg+1476+940
ARGP4
ADDRLP4 0
ARGP4
...
;2047:		{
line 2048
;2048:			VectorCopy( cg.refdef.vieworg, gunpoint );
ADDRLP4 32
ADDRGP4 cg+3616+24
INDIRB
ASGNB 12
line 2049
;2049:			AngleVectors( cg.refdefViewAngles, forward, right, NULL );
ADDRGP4 cg+3984
ARGP4
ADDRLP4 0
ARGP4
...
line 2052
;2051:
;2052:		if (weapontype == WP_EMPLACED_GUN && cg.snap->ps.emplacedIndex)
ADDRLP4 56
INDIRI4
CNSTI4 14
NEI4 $1240
...
;2053:		{
line 2054
;2054:			centity_t *gunEnt = &cg_entities[cg.snap->ps.emplacedIndex];
ADDRLP4 64
CNSTI4 1920
ADDRGP4 cg+36
INDIRP4
...
;2057:			{
line 2060
;2058:				vec3_t pitchConstraint;
;2059:
;2060:				VectorCopy(gunEnt->lerpOrigin, gunpoint);
ADDRLP4 32
ADDRLP4 64
INDIRP4
CNSTI4 928
...
;2064:				{
line 2065
;2065:					VectorCopy(cg.predictedPlayerEntity.lerpAngles, pitchConstraint);
ADDRLP4 68
ADDRGP4 cg+1476+940
INDIRB
ASGNB 12
...
;2068:				{
line 2069
;2069:					VectorCopy(cg.refdefViewAngles, pitchConstraint);
ADDRLP4 68
ADDRGP4 cg+3984
INDIRB
ASGNB 12
...
line 2072
;2071:
;2072:				if (pitchConstraint[PITCH] > 40)
ADDRLP4 68
INDIRF4
CNSTF4 1109393408
LEF4 $1253
...
LABELV $1253
line 2076
;2076:				AngleVectors( pitchConstraint, forward, right, NULL );
ADDRLP4 68
ARGP4
ADDRLP4 0
ARGP4
...
line 2080
;2079:
;2080:		VectorCopy(gunpoint, muzzle);
ADDRFP4 4
INDIRP4
ADDRLP4 32
INDIRB
...
line 2082
;2081:
;2082:		VectorMA(muzzle, weaponMuzzle[0], forward, muzzle);
ADDRLP4 64
ADDRFP4 4
INDIRP4
ASGNP4
...
ASGNF4
line 2083
;2083:		VectorMA(muzzle, weaponMuzzle[1], right, muzzle);
ADDRLP4 76
ADDRFP4 4
INDIRP4
ASGNP4
...
line 2085
;2084:
;2085:		if (weapontype == WP_EMPLACED_GUN && cg.snap->ps.emplacedIndex)
ADDRLP4 56
INDIRI4
CNSTI4 14
NEI4 $1262
...
LABELV $1262
line 2089
;2089:		else if (cg.renderingThirdPerson)
ADDRGP4 cg+88
INDIRI4
CNSTI4 0
EQI4 $1265
...
;2090:		{
line 2091
;2091:			muzzle[2] += cg.snap->ps.viewheight + weaponMuzzle[2];
ADDRLP4 88
ADDRFP4 4
INDIRP4
CNSTI4 8
...
;2099:	}
;2100:
;2101:	cent = &cg_entities[entityNum];
ADDRLP4 12
CNSTI4 1920
ADDRFP4 0
INDIRI4
...
;2104:	}
;2105:
;2106:	VectorCopy( cent->currentState.pos.trBase, muzzle );
ADDRFP4 4
INDIRP4
ADDRLP4 12
INDIRP4
...
line 2108
;2107:
;2108:	AngleVectors( cent->currentState.apos.trBase, forward, NULL, NULL );
ADDRLP4 12
INDIRP4
CNSTI4 60
ADDP4
...
pop
line 2109
;2109:	anim = cent->currentState.legsAnim & ~ANIM_TOGGLEBIT;
ADDRLP4 16
ADDRLP4 12
INDIRP4
CNSTI4 280
...
ASGNI4
line 2110
;2110:	if ( anim == BOTH_CROUCH1WALK || anim == BOTH_CROUCH1IDLE ) {
ADDRLP4 16
INDIRI4
CNSTI4 798
EQI4 $1275
...
LABELV $1275
line 2111
;2111:		muzzle[2] += CROUCH_VIEWHEIGHT;
ADDRLP4 52
ADDRFP4 4
INDIRP4
CNSTI4 8
...
LABELV $1273
line 2113
;2113:		muzzle[2] += DEFAULT_VIEWHEIGHT;
ADDRLP4 52
ADDRFP4 4
INDIRP4
CNSTI4 8
...
line 2116
;2115:
;2116:	VectorMA( muzzle, 14, forward, muzzle );
ADDRLP4 52
ADDRFP4 4
INDIRP4
ASGNP4
...
;2123:
;2124:/*
;2125:Ghoul2 Insert Start
;2126:*/
;2127:
;2128:// create one instance of all the weapons we are going to use so we can just copy this info into each clients gun ghoul2 object in fast way
;2129:void *g2WeaponInstances[MAX_WEAPONS];
;2130:void CG_InitG2Weapons(void)
;2131:{
line 2132
;2132:	int i = 0;
ADDRLP4 4
...
line 2134
;2133:	gitem_t		*item;
;2134:	memset(g2WeaponInstances, 0, sizeof(g2WeaponInstances));
...
pop
line 2135
;2135:	for ( item = bg_itemlist + 1 ; item->classname ; item++ ) 
ADDRLP4 0
...
;2138:		{
line 2140
;2139:			// initialise model
;2140:			trap_G2API_InitGhoul2Model(&g2WeaponInstances[/*i*/item->giTag], item->world_model[0], 0, 0, 0, 0, 0);
ADDRLP4 0
INDIRP4
CNSTI4 40
ADDP4
...
pop
line 2142
;2141://			trap_G2API_InitGhoul2Model(&g2WeaponInstances[i], item->world_model[0],G_ModelIndex( item->world_model[0] ) , 0, 0, 0, 0);
;2142:			if (g2WeaponInstances[/*i*/item->giTag])
ADDRLP4 0
INDIRP4
CNSTI4 40
ADDP4
...
;2143:			{
line 2145
;2144:				// indicate we will be bolted to model 0 (ie the player) on bolt 0 (always the right hand) when we get copied
;2145:				trap_G2API_SetBoltInfo(g2WeaponInstances[/*i*/item->giTag], 0, 0);
ADDRLP4 0
INDIRP4
CNSTI4 40
ADDP4
...
pop
line 2147
;2146:				// now set up the gun bolt on it
;2147:				trap_G2API_AddBolt(g2WeaponInstances[/*i*/item->giTag], 0, "*flash");
ADDRLP4 0
INDIRP4
CNSTI4 40
ADDP4
...
line 2162
;2159:
;2160:// clean out any g2 models we instanciated for copying purposes
;2161:void CG_ShutDownG2Weapons(void)
;2162:{
line 2164
;2163:	int i;
;2164:	for (i=0; i<MAX_WEAPONS; i++)
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $1292
...
;2165:	{
line 2166
;2166:		trap_G2API_CleanGhoul2Models(&g2WeaponInstances[i]);
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
...
line 2172
;2169:
;2170:// what ghoul2 model do we want to copy ?
;2171:void CG_CopyG2WeaponInstance(int weaponNum, void *toGhoul2)
;2172:{
line 2174
;2173:	//rww - the -1 is because there is no "weapon" for WP_NONE
;2174:	assert(weaponNum < MAX_WEAPONS);
line 2175
;2175:	if (g2WeaponInstances[weaponNum/*-1*/])
ADDRFP4 0
INDIRI4
CNSTI4 2
LSHI4
...
;2176:	{
line 2177
;2177:		if (weaponNum == WP_EMPLACED_GUN)
ADDRFP4 0
INDIRI4
CNSTI4 14
NEI4 $1299
line 2178
;2178:		{ //a bit of a hack to remove gun model when using an emplaced weap
line 2179
;2179:			if (trap_G2API_HasGhoul2ModelOnIndex(&(toGhoul2), 1))
ADDRFP4 4
ARGP4
CNSTI4 1
ARGI4
...
;2180:			{
line 2181
;2181:				trap_G2API_RemoveGhoul2Model(&(toGhoul2), 1);
ADDRFP4 4
ARGP4
CNSTI4 1
ARGI4
...
;2185:		{
line 2186
;2186:			trap_G2API_CopySpecificGhoul2Model(g2WeaponInstances[weaponNum/*-1*/], 0, toGhoul2, 1); 
ADDRFP4 0
INDIRI4
CNSTI4 2
LSHI4
...
line 2192
;2190:
;2191:void CG_CheckPlayerG2Weapons(playerState_t *ps, centity_t *cent) 
;2192:{
line 2194
;2193:	// should we change the gun model on this player?
;2194:	if (cent->currentState.saberInFlight)
ADDRFP4 4
INDIRP4
CNSTI4 228
ADDP4
...
;2195:	{
line 2196
;2196:		cent->ghoul2weapon = g2WeaponInstances[WP_SABER];
ADDRFP4 4
INDIRP4
CNSTI4 960
ADDP4
...
line 2199
;2198:
;2199:	if (cent->currentState.eFlags & EF_DEAD)
ADDRFP4 4
INDIRP4
CNSTI4 8
ADDP4
...
EQI4 $1307
line 2200
;2200:	{ //no updating weapons when dead
line 2201
;2201:		cent->ghoul2weapon = NULL;
ADDRFP4 4
INDIRP4
CNSTI4 960
ADDP4
...
EQI4 $1309
line 2206
;2206:	{ //got our limb cut off, no updating weapons until it's restored
line 2207
;2207:		cent->ghoul2weapon = NULL;
ADDRFP4 4
INDIRP4
CNSTI4 960
ADDP4
...
;2220:	}
;2221:
;2222:	if (cent->ghoul2 && cent->ghoul2weapon != g2WeaponInstances[ps->weapon] &&
ADDRLP4 4
ADDRFP4 4
INDIRP4
ASGNP4
...
NEI4 $1315
line 2224
;2223:		ps->clientNum == cent->currentState.number) //don't want spectator mode forcing one client's weapon instance over another's
;2224:	{
line 2225
;2225:		CG_CopyG2WeaponInstance(ps->weapon, cent->ghoul2);
ADDRFP4 0
INDIRP4
CNSTI4 148
ADDP4
...
pop
line 2226
;2226:		cent->ghoul2weapon = g2WeaponInstances[ps->weapon];
ADDRFP4 4
INDIRP4
CNSTI4 960
ADDP4
...
ASGNP4
line 2227
;2227:		if (cent->weapon == WP_SABER && cg_entities[cent->currentState.number].weapon != ps->weapon && !ps->saberHolstered)
ADDRLP4 12
ADDRFP4 4
INDIRP4
ASGNP4
...
NEI4 $1317
line 2228
;2228:		{ //switching away from the saber
line 2229
;2229:			trap_S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, trap_S_RegisterSound( "sound/weapons/saber/saberoffquick.wav" ));
ADDRGP4 $1320
ARGP4
ADDRLP4 20
ADDRGP4 trap_S_RegisterSound
...
LABELV $1317
line 2231
;2231:		else if (ps->weapon == WP_SABER && cg_entities[cent->currentState.number].weapon != ps->weapon)
ADDRLP4 20
ADDRFP4 0
INDIRP4
CNSTI4 148
...
EQI4 $1321
line 2232
;2232:		{ //switching to the saber
line 2233
;2233:			trap_S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, trap_S_RegisterSound( "sound/weapons/saber/saberon.wav" ));
ADDRGP4 $1324
ARGP4
ADDRLP4 24
ADDRGP4 trap_S_RegisterSound
...
ASGNI4
line 2236
;2236:		cg_entities[cent->currentState.number].weapon = ps->weapon;
CNSTI4 1920
ADDRFP4 4
INDIRP4
INDIRI4
